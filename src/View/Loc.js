...
var image = "https://t1.daumcdn.net/localimg/localimages/07/mapapidoc/markerStar.png"; 
  <React.Fragment>
  {
       ...
       props.state.pinfo.map((item) => {
        // 지도에 마커를 표시하는 함수입니다
  function displayMarkerP(item) {
    // 마커의 이미지정보를 가지고 있는 마커이미지를 생성합니다
  ...
  var marker = new kakao.maps.Marker({
    ...
    position: new kakao.maps.LatLng(item.lat, item.lon), // 마커를 표시할 위치
    image: markerImage, // 마커이미지 설정 
    title :item.ppname, // 마커의 타이틀, 마커에 마우스를 올리면 타이틀이 표시됩니다
  });

  // 마커에 클릭이벤트를 등록합니다
  kakao.maps.event.addListener(marker, 'click', function() {
      // 마커를 클릭하면 장소명이 인포윈도우에 표출됩니다
      infowindow.setContent('<div style="padding:5px;font-size:12px;width:300px;">' + item.ppname+'<br/>'+item.ppaddress1+'</div>');
      infowindow.open(map, marker);
  });
       }})
      }
